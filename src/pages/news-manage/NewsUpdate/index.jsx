...
export default function NewsUpdate() {
  ...
  const steps = [
    ...
    {
      title: '新闻内容',
      content: <Editor editorState={editorState} onEditorStateChange={state => setEditorState(state)}
  ...
  useEffect(() => {
    ...
    axios.get(`/news/${params.id}?_expand=category&_expand=role`).then(res => {
      ...
      //转换内容数据
      if(!content)return
      const blocksFromHtml = htmlToDraftjs(content);
  ...
  return (
    <div>
      {/* 页头 */}
      <PageHeader title="更新新闻" subTitle="This is a subtitle" onBack={() => navigate(-1)} />
      {/* 步骤条 */}
